...
struct comcerto_xor_chan {
	...
	struct list_head            free_sw_descs;
};
...
#define XOR_SW_FDESC_COUNT (XOR_FDESC_COUNT / SPLIT_SIZE)
...
static int comcerto_xor_set_desc(struct comcerto_sw_xor_desc *sw_desc, int hw_idx)
{
	int i,split_no;
	u32 fstatus0 = 0;
	u32 addr;
	...
	block_size = sw_desc->len;
	src_cnt = sw_desc->src_cnt;
	srcs = sw_desc->dma_src;
	dest = sw_desc->dma_dest;
	...
	for(split_no = 0 ; split_no < SPLIT_SIZE; split_no++)
	{
		...
		for(i = 0; i < src_cnt - 1; i++) {
			addr = (u32)sw_desc->dma_src[i] + 4096 * split_no;
			comcerto_xor_set_in_bdesc(hw_idx, i, addr, block_size);
		}

		addr = (u32)sw_desc->dma_src[src_cnt - 1] + 4096 * split_no;
		...
		addr = (u32)sw_desc->dma_dest + 4096 * split_no;
		comcerto_xor_set_out_bdesc(hw_idx, 0, addr, block_size | BLAST);

		xor_out_fdesc[hw_idx]->fcontrol = (u32)sw_desc;
...
{
	if(list_empty(&comcerto_xor_ch.free_sw_descs) || (CIRC_SPACE(xor_reserve_idx, xor_rd_idx, XOR_FDESC_COUNT) < SPLIT_SIZE))
		return 1;
	else
		return 0;
}
...
static void comcerto_xor_cleanup(void)
{
	...
	if(cleanup_count && (cleanup_count % SPLIT_SIZE== 0))
	{
		for(i = 0 ; i < cleanup_count/SPLIT_SIZE; i++)
		{
			...
			if(xor_rd_idx % SPLIT_SIZE)
				printk("%s: xor_rd_idx %d not multiple of %d\n",__func__, xor_rd_idx, SPLIT_SIZE);

			idx = xor_rd_idx;
			sw_desc = (struct comcerto_sw_xor_desc *)xor_out_fdesc[idx]->fcontrol;
			tx = &sw_desc->async_tx;
			...
			xor_rd_idx = (xor_rd_idx + SPLIT_SIZE) % XOR_FDESC_COUNT;
			
			list_add(&sw_desc->list, &comcerto_xor_ch.free_sw_descs);
			spin_unlock_irqrestore(&mdma_lock, flags);
		}
	}
	else
...
static dma_cookie_t comcerto_xor_tx_submit(struct dma_async_tx_descriptor *tx)
{
	...
	xor_wr_idx = comcerto_xor_set_desc(sw_desc, xor_wr_idx);
	comcerto_dma_process();
		...
		unsigned int src_cnt, size_t len, unsigned long xor_flags)
{
	...
	//reserve the hw descriptors
	xor_reserve_idx = (xor_reserve_idx + SPLIT_SIZE) % XOR_FDESC_COUNT;

	//allocate the sw descriptors from the list by removing from it
	sw_desc = list_first_entry(&comcerto_xor_ch.free_sw_descs, struct comcerto_sw_xor_desc, list);
	list_del(&sw_desc->list);

	sw_desc->async_tx.flags = xor_flags;
	sw_desc->src_cnt = src_cnt;
	sw_desc->len = len;
	sw_desc->dma_dest = dest;
	for(i = 0; i < src_cnt; i++)
		sw_desc->dma_src[i] = srcs[i];
...
static int comcerto_xor_alloc_chan_resources(struct dma_chan *chan)
{
	...
	for(i = 0; i < XOR_SW_FDESC_COUNT; i++) {
		sw_desc = kzalloc(sizeof(struct comcerto_sw_xor_desc), GFP_KERNEL);
		sw_desc->async_tx.tx_submit = comcerto_xor_tx_submit;
		sw_desc->async_tx.cookie = 0;

		dma_async_tx_descriptor_init(&sw_desc->async_tx, chan);

		list_add_tail(&sw_desc->list, &comcerto_xor_ch.free_sw_descs);
	}
...
{
	struct comcerto_sw_xor_desc *sw_desc, *__sw_desc;

	spin_lock_bh(&comcerto_xor_ch.lock);

	list_for_each_entry_safe(sw_desc, __sw_desc, &comcerto_xor_ch.free_sw_descs, list)
	{
		list_del(&sw_desc->list);
		kfree(sw_desc);
	}
...
struct mdma_stats mdma_stats;
unsigned int mdma_stats_enable = 0;
#endif
...
static int __devinit comcerto_dma_probe(struct platform_device *pdev)
{
	...
	INIT_LIST_HEAD(&comcerto_xor_ch.free_sw_descs);
